...
import { LOGIN_AUTH_USER } from "../../graphql";
...
const LoginComponent = () => {
  ...
  const { loading, error, data } = useQuery(LOGIN_AUTH_USER, {
  ...
  const handleOnSubmit = (e) => {
    ...
    e.preventDefault();
    if (
      (userData?.email === email && checkedPassword) ||
      (userData?.email === email && password == userData?.password)
    ) {
  ...
  return (
    <section className="max-w-5xl mx-auto my-0 bg-gray-50 dark:bg-gray-900">
      <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        <a
          href="#"
          className="flex items-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white"
        >
          <img className="h-32 mr-2 w-80" src={logo} alt="logo" />
        </a>
