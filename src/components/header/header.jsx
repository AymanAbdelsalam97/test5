...
import { GET_AUTHUSER_PROFILE } from "../../graphql";
...
const Header = () => {
  ...
  const { loading, error, data } = useQuery(GET_AUTHUSER_PROFILE, {
    variables: { email: token.email },
  });
  const userData = data?.authUser;
  const profilePicture = userData?.profile?.profilePicture?.url;
  const handleLogout = () => {
    logout();
    navigate("/login");
  };
